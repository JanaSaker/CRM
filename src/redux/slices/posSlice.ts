...
const posSlice = createSlice({
  ...
  reducers: {
    addProductToCart(
      ...
      const existingCartItem = state.activeInvoice.cart.find(
    ...
    removeProductFromCart(state, action: PayloadAction<string>) {
      ...
      if (state.activeInvoice) {
        state.activeInvoice.cart = state.activeInvoice.cart.filter(
        ...
        state.activeInvoice.totalPrice = state.activeInvoice.cart.reduce(
          (total, item) =>
            total +
            item.quantity *
              (item.prices[state.activeInvoice?.priceCategory || 1] || 0),
          0
        );
      }
    },
    ...
    updateSelectedRow(
      ...
      if (cartItem) {
        ...
        cartItem.subtotal =
          cartItem.quantity * price * (1 + vat / 100) * (1 - discount / 100);
    ...
    updateInvoiceCurrency(
      state,
      action: PayloadAction<{ id: number; usdtocurr: number }>
    ) {
      if (!state.activeInvoice) return;
      ...
      activeInvoice.cart.forEach((item) => {
        ...
        item.subtotal =
          item.quantity * price * (1 + vat / 100) * (1 - discount / 100);
      });

      // Recalculate the total price
      activeInvoice.totalPrice = activeInvoice.cart.reduce(
        (total, item) => total + item.subtotal,
        0
      );
      ...
      if (existingDraftIndex !== -1) {
        state.invoiceDrafts[existingDraftIndex].selectedCurrency =
          action.payload;
      }
    },
    ...
    setActiveInvoice(state, action: PayloadAction<string>) {
      ...
      const newActiveDraft = state.invoiceDrafts.find(
    ...
    updateInvoiceDiscount(state, action: PayloadAction<number>) {
      ...
      const { usdtocurr } = state.activeInvoice.selectedCurrency || {
        usdtocurr: 1,
      };
...
export const selectCartTotal = (state: { pos: POSState }) =>
  state.pos.cart.reduce(
    (total, item) => total + (item.prices[1] || 0) * item.quantity,
    0
  );

export const selectInvoiceDrafts = (state: { pos: POSState }) =>
  state.pos.invoiceDrafts;

export const selectActiveInvoice = (state: { pos: POSState }) =>
  state.pos.activeInvoice;
